...
3. **crypto-shredding（鍵破棄）で実質的な復元不能を担保する。**
   ...
   - **【実装時に判明した修正】データ鍵を暗号文と同じデータベースに置くと、この対策は成立しない。**
     バックアップには暗号文と鍵の両方が入るため、DBを丸ごと復元されれば復号できてしまう。
     鍵ストアは **DB外（KMS または外部KV）** に置くこと。
     現在の実装（`event_keys` テーブル）は暫定であり、テーブル漏洩・削除後の行復元には有効だが、
     DBバックアップ経路には無力である。訴求文言をこの実態に合わせるか、鍵ストアを外部化するかを選ぶこと
