...
const IMAGE_CATEGORIES = {
  star: { count: parseInt(process.env.STAR_IMAGE_COUNT || '8'), startNum: 301 },
};
...
export async function GET(request) {
  ...
  try {
    ...
    if (!noImage) {
      ...
      mediaId = await uploadPhotoToX(xClient, 'star', next);
      photoMeta = { index: next, photoKey, uploaded: !!mediaId };
      const shownNum = String(IMAGE_CATEGORIES.star.startNum + next).padStart(5, '0');
      report.image = mediaId ? `添付成功 (star/${shownNum}.jpg)` : `添付失敗（画像なしで投稿続行） (star/${shownNum}.jpg)`;
    }
