...
export async function GET(request) {
  ...
  ? `[Sakura Season Rules]
桜シーズン（2月1日から4月15日）です。
2月1日からの積算温度を推定し、以下の基準でミゴロン指数を算出すること。
開花目安：積算温度210度（指数50%前後）
満開目安：積算温度370度（指数90%以上）
関東・近郊の実在する桜の名所5件を選び、標高・地域差を考慮して指数に差をつけること。`
  : `[Seasonal Flower Rules]
今の時期（${dateLabel}）に関東・近郊で見頃の花を選ぶこと。
旬の花：${flowers.join(', ')}
上記の花が咲いている関東・近郊の実在する名所を5件選ぶこと。
積算温度の計算は不要。現在の季節感からミゴロン指数を算出すること。`;
