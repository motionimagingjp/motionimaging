...
async function buildFujisanTweet(apiKey, dateLabel, weather, penalty, min) {
  const prompt = '明日早朝の富士山撮影スポット3件のミゴロン指数を算出してください。\n'
    ...
    + '条件：富士山周辺の実在する場所3件\n\n'
    + '必ず以下のJSON形式のみで返してください。マークダウン不要。\n'
    + '{"spots":[{"name":"新道峠（山梨）","emoji":"🗻","score":90},{"name":"田貫湖（静岡）","emoji":"🗻","score":85},{"name":"山中湖パノラマ台（山梨）","emoji":"🗻","score":75}],"time":"5:00〜7:00","memo":"明日の富士山撮影ポイントを一言で"}';
  const raw = await callGemini(apiKey, prompt);
  const clean = raw.replace(/```json|```/g, '').trim();
  const match = clean.match(/\{[\s\S]*\}/);
  let spots, time, memo;
  if (match) {
    ...
    spots = [
      { name: '新道峠（山梨）',           emoji: '🗻', score: Math.max(10, 90 - penalty) },
...
export async function GET(request) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ message: 'Success', date: dateLabel, weather, results }), { status: 200 });
  } catch (error) {
