  ...
  try {
    ...
    if (mediaData.data && mediaData.data.length > 0) {
      ...
      if (lastPostDateStr === today) {
        console.log('⚠️ Instagram API: Already posted today, skipping');
        await redis.set(todayKey, today, { ex: 90000 });
        return new Response(JSON.stringify({ message: '本日投稿済みのためスキップ（Instagram API）' }), { status: 200 });
      }
    }
  } catch (e) {
    console.error('Instagram media check failed:', e.message);
  }
